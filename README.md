...
### 다수의 사용자가 동시에 검색을 요청하는 경우 트랜잭션 단위를 어떻게 설정해야 할까?

![transaction](images/transaction.png)

블로그 검색 이후 키워드를 저장할 때 기존의 키워드의 검색 횟수 update 가 아닌 새로운 키워드 검색 횟수 insert 시점에 동시성 문제가 발생할 수 있다고 판단했습니다.
해당 문제는 `조회 -> 저장, 조회 -> 수정` 으로 트랜잭션 단위를 설계하고 Redisson 분산 락을 활용하여 동시성 문제를 해결하였습니다.
