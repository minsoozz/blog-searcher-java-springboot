...
public class CallerCircuitBreakerImpl implements CallerCircuitBreaker {
    ...
    public ApiResponseDto defaultFallback(final String query, final Integer page, final String sort) {
        ...
        try {
            ResponseEntity<String> response = restTemplate.exchange(
                ...
                entity,
                String.class);
            return objectMapperUtils.jsonToObject(response.getBody(), FallbackApiResponseDto.class);
    ...
    private String convertSortOrderToFallbackSortString(final String sort) {
        return switch (sort) {
            case "recency" -> "date";
            default -> "sim";
        };
